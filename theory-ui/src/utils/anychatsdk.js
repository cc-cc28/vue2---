...
import {
  OnAnyChatCoreSDKEvent,
  OnAnyChatNotifyMessage,
  OnAnyChatObjectEvent,
  OnAnyChatRecordSnapShot,
  OnAnyChatRecordSnapShotEx,
  OnAnyChatRecordSnapShotEx2,
  OnAnyChatTextMessage,
  OnAnyChatTransBuffer, OnAnyChatTransBufferEx, OnAnyChatTransFileEx,
  OnAnyChatVideoCallEvent
} from "@/utils/anychatevent";
...
export var BRAC_DEVICE_VIDEOCAPTURE                = 1; // 视频采集设备
export var BRAC_DEVICE_AUDIOCAPTURE                = 2; // 音频采集设备
...
export var BRAC_SO_CORESDK_BUILDTIME               = 24; // 查询SDK编译时间（参数为字符串TCHAR类型）
...
export var BRAC_SO_CORESDK_NEWGUID                 = 29; // 产生新的GUID字符串

export var BRAC_SO_LOCALVIDEO_BITRATECTRL          = 30; // 本地视频编码码率设置（参数为int型，单位bps，同服务器配置：VideoBitrate）
...
export var BRAC_SO_LOCALVIDEO_FPSCTRL              = 33; // 本地视频编码帧率控制（参数为int型，同服务器配置：VideoFps）
var BRAC_SO_LOCALVIDEO_PRESETCTRL           = 34; // 本地视频编码预设参数控制（参数为int型，1-5）
export var BRAC_SO_LOCALVIDEO_APPLYPARAM           = 35; // 应用本地视频编码参数，使得前述修改即时生效（参数为int型：1 使用新参数，0 使用默认参数）
...
export var BRAC_SO_LOCALVIDEO_WIDTHCTRL            = 38; // 本地视频采集分辨率宽度控制（参数为int型，同服务器配置：VideoWidth）
export var BRAC_SO_LOCALVIDEO_HEIGHTCTRL           = 39; // 本地视频采集分辨率高度控制（参数为int型，同服务器配置：VideoHeight）
...
export var BRAC_SO_LOCALVIDEO_DEVICENAME           = 106; // 本地视频采集设备名称，用于设置打开指定摄像头设备（参数为字符串类型）
...
export var BRAC_SO_CORESDK_WRITELOG                = 135; // 写入调试信息到本地日志文件中
...
export var BRAC_SO_H5WEBSDK_VERSION                = 1000; //H5SDK版本设置（1 适配器版本（默认）, 2 原始版本）,在调用BRAC_Connect接口前设置
export var BRAC_SO_H5WEBSDK_BLOBDATA               = 1001; //获取转换blob数据块
...
export var BRAC_STREAMINFO_VIDEOCODECID            = 184; // 视频流编码器ID
...
export var BRAC_STREAMINFO_AUDIOFLAG               = 501; //音频媒体标志(请求音视频前调用， 1:开启音频 0:不开启音频)
export var BRAC_STREAMINFO_VIDEOFLAG               = 502; //视频媒体标志(请求音视频前调用， 1:开启视频 0:不开启视频)

var BRAC_SO_OBJECT_INITFLAGS                = 200; // 业务对象身份初始化
export var BRAC_SO_CLOUD_APPGUID                   = 300; // 云平台应用GUID（参数为：字符串类型，连接服务器之前设置）
...
export var BRAC_USERSTATE_CAMERA                   = 1; // 用户摄像头状态（参数为DWORD型）
export var BRAC_USERSTATE_HOLDMIC                  = 2; // 用户音频设备状态（参数为DWORD型，返回值：0 音频采集关闭， 1 音频采集开启）
var BRAC_USERSTATE_SPEAKVOLUME              = 3; // 用户当前说话音量（参数为DOUBLE类型（0.0 ~ 100.0））
var BRAC_USERSTATE_RECORDING                = 4; // 用户录像（音）状态（参数为DWORD型）
export var BRAC_USERSTATE_LEVEL                    = 5; // 用户级别（参数为DWORD型）
export var BRAC_USERSTATE_NICKNAME                 = 6; // 用户昵称（参数为字符串TCHAR类型）
var BRAC_USERSTATE_LOCALIP                  = 7; // 用户本地IP地址（内网，参数为字符串TCHAR类型）
var BRAC_USERSTATE_INTERNETIP               = 8; // 用户互联网IP地址（参数为字符串TCHAR类型）
export var BRAC_USERSTATE_VIDEOBITRATE             = 9; // 用户当前的视频码率（参数为DWORD类型，Bps）
export var BRAC_USERSTATE_AUDIOBITRATE             = 10; // 用户当前的音频码率（参数为DWORD类型，Bps）
...
export var BRAC_USERSTATE_VIDEOSIZE                = 13; // 查询指定用户的视频分辨率（参数为DWORD类型，返回值：低16位表示宽度，高16位表示高度）
...
export var WM_GV_CONNECT                           = WM_GV + 1; // 客户端连接服务器，wParam（BOOL）表示是否连接成功
export var WM_GV_LOGINSYSTEM                       = WM_GV + 2; // 客户端登录系统，wParam（INT）表示自己的用户ID号，lParam（INT）表示登录结果：0 成功，否则为出错代码，参考出错代码定义
export var WM_GV_ENTERROOM                         = WM_GV + 3; // 客户端进入房间，wParam（INT）表示所进入房间的ID号，lParam（INT）表示是否进入房间：0成功进入，否则为出错代码
export var WM_GV_MICSTATECHANGE                    = WM_GV + 4; // 用户的音频设备状态变化消息，wParam（INT）表示用户ID号，lParam（BOOL）表示该用户是否已打开音频采集设备
export var WM_GV_USERATROOM                        = WM_GV + 5; // 用户进入（离开）房间，wParam（INT）表示用户ID号，lParam（BOOL）表示该用户是进入（TRUE）或离开（FALSE）房间
export var WM_GV_LINKCLOSE                         = WM_GV + 6; // 网络连接已关闭，该消息只有在客户端连接服务器成功之后，网络异常中断之时触发，wParam（INT）表示连接断开的原因
export var WM_GV_ONLINEUSER                        = WM_GV + 7; // 收到当前房间的在线用户信息，进入房间后触发一次，wParam（INT）表示在线用户数（包含自己），lParam（INT）表示房间ID
export var WM_GV_CAMERASTATE                       = WM_GV + 11; // 用户摄像头状态发生变化，wParam（INT）表示用户ID号，lParam（INT）表示摄像头的当前状态，定义为：GV_CAMERA_STATE_XXXX
export var WM_GV_CHATMODECHG                       = WM_GV + 12; // 用户聊天模式发生变化，wParam（INT）表示用户ID号，lParam（INT）表示用户的当前聊天模式
export var WM_GV_ACTIVESTATE                       = WM_GV + 13; // 用户活动状态发生变化，wParam（INT）表示用户ID号，lParam（INT）表示用户的当前活动状态
export var WM_GV_P2PCONNECTSTATE                   = WM_GV + 14; // 本地用户与其它用户的P2P网络连接状态发生变化，wParam（INT）表示其它用户ID号，lParam（INT）表示本地用户与其它用户的当前P2P网络连接状态
export var WM_GV_VIDEOSIZECHG		                = WM_GV + 15; // 用户视频分辩率发生变化，wParam（INT）表示用户ID号，lParam（INT）表示用户的视频分辨率组合值（低16位表示宽度，高16位表示高度）
export var WM_GV_USERINFOUPDATE	                = WM_GV + 16; // 用户信息更新通知，wParam（INT）表示用户ID号，lParam（INT）表示更新类别
export var WM_GV_FRIENDSTATUS		                = WM_GV + 17; // 好友在线状态变化，wParam（INT）表示好友用户ID号，lParam（INT）表示用户的当前活动状态：0 离线， 1 上线
export var WM_GV_OBJECTEVENT		                = WM_GV + 18; // 对象事件通知，wParam（INT）表示对象类型，lParam（INT）表示对象ID
export var WM_GV_SESSIONKEEP		                = WM_GV + 19; // 会话保持事件，wParam（INT）表示会话保持状态：0 会话保持开始， 1 会话保持结束，lParam（INT）表示出错代码
export var WM_GV_PRIVATEREQUEST                    = WM_GV + 21; // 用户发起私聊请求，wParam（INT）表示发起者的用户ID号，lParam（INT）表示私聊请求编号，标识该请求
export var WM_GV_PRIVATEECHO                       = WM_GV + 22; // 用户回复私聊请求，wParam（INT）表示回复者的用户ID号，lParam（INT）为出错代码
export var WM_GV_PRIVATEEXIT                       = WM_GV + 23; // 用户退出私聊，wParam（INT）表示退出者的用户ID号，lParam（INT）为出错代码
export var WM_GV_SERVERCFGUPDATE	                = WM_GV + 24; // 服务器配置参数更新

export var WM_GV_EXTENDBTNPRESS	                = WM_GV + 31; // 用户按下扩展按钮，wParam（INT）表示按钮所对应的用户ID号，lParam（DWORD）指示按钮（左下角）所在屏幕位置(x,y)，用户可以利用该参数显示菜单等
export var WM_GV_VIDEOFULLSCREEN                   = WM_GV + 32; // 用户视频全屏通知，wParam（INT）表示用户ID，lParam(DWORD)表示当前视频是否全屏显示
export var WM_GV_SDKWARNING                        = WM_GV + 41; // SDK警告信息，当SDK在运行过程中自检发现异常状态时，将向上层发送该消息，wParam（INT）表示警告代码，定义为：GV_ERR_WARNING_XXXX
export var WM_GV_AUDIOPLAYCTRL		                = WM_GV + 100; // 音频播放设备控制，wParam:1 打开播放器， 0 关闭播放器， lParam：profile，android平台内部使用
export var WM_GV_AUDIORECCTRL		                = WM_GV + 101; // 音频录音设备控制，wParam:1 打开音频采集设备，0 关闭音频采集设备
export var WM_GV_VIDEOCAPCTRL		                = WM_GV + 102; // 视频采集控制，wParam: 1 开始采集， 0 关闭采集
export var WM_GV_ICELINKCLOSE                      = WM_GV + 500; // 用户网络异常，音视频链接建立失败，wParam（INT）表示用户ID号, lParam（INT）表示连接结果：0 成功，-1 失败
export var WM_GV_MEDIAAUTHORITY                    = WM_GV + 501; // 获取媒体权限结果，wParam（INT）表示用户成功与否，0 成功, -1 失败， lParam（string） 表示错误信息
export var WM_GV_H5MEDIAOPENSTATUS                 = WM_GV + 502; // H5媒体状态，wParam（INT）表示媒体开启状态成功与否，0 成功, -1 失败， lParam（string） 表示错误信息
export var WM_GV_QUERYINFOFORMSERVER				= WM_GV + 503; // 服务器信息查询,wParam 表示服务器返回的查询结果，lParam 为出错代码：0 成功
export var WM_GV_STREAMRECORDCTRLEX				= WM_GV + 504; // BRAC_StreamRecordCtrlEx接口执行结果，lParam 为出错代码：0 成功
...
export var BRAC_VIDEOCALL_EVENT_REQUEST            = 1; // 呼叫请求
export var BRAC_VIDEOCALL_EVENT_REPLY              = 2; // 呼叫请求回复
export var BRAC_VIDEOCALL_EVENT_START              = 3; // 视频呼叫会话开始事件
export var BRAC_VIDEOCALL_EVENT_FINISH             = 4; // 挂断（结束）呼叫会话
...
export var GV_ERR_BROWSERNOWEBRTC                  = 1020000; // 不支持webrtc
...
export function BRAC_InitSDK(apilevel) {
...
// 设置视频显示位置
export function BRAC_SetVideoPos(userid, parentobj, id) {
...
// 连接服务器
export function BRAC_Connect(lpServerAddr, dwPort) {
    return anychat.Connect(lpServerAddr, dwPort);
}
// 登录系统
export function BRAC_Login(lpUserName, lpPassword, dwParam) {
...
// 进入房间
export function BRAC_EnterRoom(dwRoomid, lpRoomPass, dwParam) {
...
// 离开房间
export function BRAC_LeaveRoom(dwRoomid) {
...
export function BRAC_GetOnlineUser() {
    if (bSupportScriptObject) {
...
export function BRAC_GetRoomOnlineUsers(dwRoomId) {
    if (bSupportScriptObject) {
...
// 查询用户名称
export function BRAC_GetUserName(dwUserId) {
    return anychat.QueryUserStateString(dwUserId, BRAC_USERSTATE_NICKNAME);
}
// 查询指定用户相关状态（整型值状态）
export function BRAC_QueryUserStateInt(dwUserId, infoname) {
...
// 枚举本地设备信息（返回设备名称数组）
export function BRAC_EnumDevices(dwDeviceType) {
...
// 操作本地用户视频（或请求远程用户视频）
export function BRAC_UserCameraControl(dwUserId, bOpen) {
    return anychat.UserCameraControl(dwUserId, bOpen);
}

// 操作本地用户视频扩展（或请求远程用户视频）
export function BRAC_UserCameraControlEx(dwUserId, bOpen, dwStreamIndex, dwFlags, szStrParam) {
...
// 操作本地用户语音（或请求远程用户语音）
export function BRAC_UserSpeakControl(dwUserId, bOpen) {
    return anychat.UserSpeakControl(dwUserId, bOpen);
}

// 操作本地用户语音扩展（或请求远程用户语音）
export function BRAC_UserSpeakControlEx(dwUserId, bOpen, dwStreamIndex, dwFlags, szStrParam) {
    if (bSupportH5Media || bSupportMultiStream)
...
// 传送文本消息
export function BRAC_SendTextMessage(dwUserId, bSecret, lpMsgBuf) {
...
// SDK内核参数设置
export function BRAC_SetSDKOption(optname, value) {
...
// SDK内核参数状态查询（字符串参数值）
export function BRAC_GetSDKOptionString(optname) {
...
// 视频呼叫事件控制（请求、回复、挂断等）
export function BRAC_VideoCallControl(dwEventType, dwUserId, dwErrorCode, dwFlags, dwParam, szUserStr) {
...
export function BRAC_SetUserStreamInfo(dwUserId, dwStreamIndex, infoname, value) {
    if (bSupportH5Media)
    {
